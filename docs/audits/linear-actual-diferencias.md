...
- `apps/server/src/db/migrations/0001_init.sql` a `0023_webhook_team_scope.sql` (snapshot histórico);
  el runner SQLite vigente llega a `0027_documents.sql`.
...
- El runner de SQLite en `apps/server/src/db/database.ts` aplica las migraciones `0001` a `0027`.
  `0024` y `0025` agregan raíces, columnas `workspace_id` e invariantes compuestas; `0026` agrega
  grants de API keys por Workspace y `0027` agrega Documents.
- El migrador de PostgreSQL en `apps/server/src/db/postgres/migrator.ts` aplica cuatro migraciones
  independientes (`0001` a `0004`). Su baseline crea un índice singleton de Workspace y todavía no
  representa el alcance multi-Workspace de SQLite.
- En SQLite, el contrato GraphQL vigente lista y crea Workspaces y acepta selección por contexto;
  ...
  operación no está migrada y el resolver de auth conserva el singleton. Los dominios no migrados
  usan un SQLite efímero durante esta transición.
...
migraciones `0024`–`0027`. Esa capacidad todavía es incremental y no se puede trasladar al backend
...
| Identidad              | `User` tiene organización, roles, estado, memberships de Team y variantes guest/app.                               | `Actor` puede ser `human` o `agent`. SQLite aplica role/status por Membership y grants de key; PostgreSQL conserva campos globales durante la migración.                                                                | **Parcial + divergencia agent-first**.             | PRB-413 y cutover PostgreSQL                                             |
| Workspace Membership   | La pertenencia a la Workspace delimita acceso; invitaciones tienen rol y Teams.                                    | Existe en SQLite con estado, rol, invitaciones y grants de API key; el baseline PostgreSQL todavía no la modela.                                                                                                        | **Parcial por backend**.                           | PRB-413 y cutover PostgreSQL                                             |
...
| Webhooks               | Más tipos de eventos y administración; se instalan dentro de una Workspace.                                        | SQLite persiste scope de Workspace y Team; el dispatcher y el baseline PostgreSQL siguen en migración incremental.                                                                                                      | **Parcial por backend**.                           | PRB-416 y cutover PostgreSQL                                             |
| API GraphQL            | Contrato muy amplio, `Organization`, conexiones Relay y muchas mutaciones (snapshot: 161 queries y 361 mutations). | SDL local vigente: 28 queries y 67 mutations; el backend PostgreSQL aún deja operaciones en SQLite efímero.                                                                                                             | **Parcial**.                                       | PRB-414/415; paridad total no es objetivo.                               |
...
- Una misma DB puede contener varias Workspaces. `workspace_id`, Memberships, grants de API keys y
  FKs compuestas mantienen el alcance de los recursos en las migraciones `0024`–`0027`.
...
- `0002_workspace_singleton.sql` impone una única Workspace. `workspaceCreate` devuelve que la
  operación todavía no está migrada y `postgres-viewer.ts` solo autoriza el singleton.
- El migrador PG tiene cinco versiones y `0005` agrega Documents. El baseline todavía no contiene Memberships/grants
  equivalentes a SQLite. El servidor usa un SQLite efímero para dominios todavía no migrados.
...
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| DB/dominio          | SQLite tiene el modelo operativo y las migraciones `0001`–`0027`; PostgreSQL tiene baseline más cuatro migraciones y cobertura incremental. | Menor cobertura de metadatos y recursos avanzados frente a Linear.                             | El alcance multi-Workspace vigente es SQLite; no asumir cutover PostgreSQL. |
...
| CLI                 | `pb` cubre issues, planning, actores, keys, webhooks y selección de Workspace en el camino SQLite.                                          | Linear no ofrece un CLI oficial equivalente; el CLI local es una ventaja agent-first.          | El camino PostgreSQL requiere completar grants y selector.                  |
...
| Auditoría/operación | Activity de Issue, webhooks, comentarios e Inbox receipts.                                                                                  | Linear separa history, notifications y audit entries con más canales.                          | Mantener el modelo compacto y scoped en SQLite; revisar PG.                 |
...
- `docs/audits/linear-modelo-datos.md` conserva el snapshot de migraciones SQLite hasta `0018`. El
  runner SQLite actual llega a `0027`; el migrador PostgreSQL es independiente y llega a `0004`.
