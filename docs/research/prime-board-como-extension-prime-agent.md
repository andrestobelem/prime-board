...
El server importa las 30 migraciones SQLite como texto durante el build. El migrador PostgreSQL importa
once migraciones independientes. `0010` agrega `projector_checkpoints` y `0011` retira Documents después
...
- [`apps/server/src/db/postgres/migrator.ts`](../../apps/server/src/db/postgres/migrator.ts): migraciones PostgreSQL `0001`–`0011` y checksums; `0010` crea `projector_checkpoints` y `0011` retira Documents tras validar el archivo externo.
