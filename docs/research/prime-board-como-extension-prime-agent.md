...
Teams, Issues, Relations, Projects, Milestones, Cycles, Labels, Activity, suscriptores, Reviews, Initiatives,
Project Updates, Saved Views, Favorites, Inbox y Webhooks. Documents fue un dominio histórico y quedó
retirado; `0011` elimina su tabla después de validar el archivo externo. Relations está implementado por
...
| SQLite                                      | `bun:sqlite` en `apps/server/src/db/database.ts`                                                                       | Backend predeterminado; Bun obligatorio para ejecutar fuente; aplica migraciones SQLite `0001`–`0030`. `0030` retira Documents tras validar el archivo externo.                                                                                                                                                                                      |
| PostgreSQL                                  | `Bun.SQL`, `apps/server/src/db/postgres/`                                                                              | Backend opcional; migrador independiente `0001`–`0011` y `0016`, singleton de Workspace y cobertura incremental. `0010` agrega `projector_checkpoints` y `0011` retira Documents tras validar el archivo externo. Paths directos incluyen Issues, Relations, API keys y límites. Comments y la proyección completa del event log canónico siguen fuera de PG. |
...
El server importa las 30 migraciones SQLite como texto durante el build. El migrador PostgreSQL registra las versiones
`0001`–`0011` y `0016` como migraciones independientes. `0010` agrega `projector_checkpoints` y `0011` retira Documents después
de validar un archivo externo; `0008` y `0009` conservan el Workspace de los límites de Team de API keys
...
- [`apps/server/src/db/database.ts`](../../apps/server/src/db/database.ts): `bun:sqlite`, WAL, migraciones SQLite `0001`–`0030` y foreign keys; `0030` retira Documents tras validar el archivo externo.
- [`apps/server/src/db/postgres/migrator.ts`](../../apps/server/src/db/postgres/migrator.ts): migraciones PostgreSQL `0001`–`0011` y `0016`, con checksums; `0010` crea `projector_checkpoints` y `0011` retira Documents tras validar el archivo externo.
