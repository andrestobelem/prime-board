...
El adaptador valida `visibility` y `access_policy` con las mismas reglas que SQLite: un Team privado exige `team_members`. PostgreSQL aplica el alcance en las lecturas de Teams: los admins descubren todos, los Teams públicos son descubribles y los privados solo aparecen para Memberships activas. Labels, Projects, Milestones, Cycles, Relations, Activity, API keys y límites de Team tienen paths PostgreSQL directos. Documents fue un dominio histórico de `0005` y quedó retirado por `0011`; no tiene un path PostgreSQL operativo. Relations se implementó en PRB-437 y API keys/límites en PRB-552 (`0008`/`0009`). Comments no tiene persistencia PostgreSQL. Las operaciones que todavía no tienen un path PG fallan explícitamente o usan el SQLite efímero de compatibilidad, según el dominio.
