  ...
  if (action === "view") {
    ...
    if (issue.relations.length > 0) {
      ...
      for (const relation of issue.relations) {
        console.log(`  ${RELATION_TEXT[relation.type] ?? relation.type} ${relation.relatedIssue.identifier}  ${relation.relatedIssue.title}`);
      }
    }
  ...
  if (action === "link" || action === "unlink") {
    ...
    if (action === "link") {
      for (const request of requests) {
        await gqlRequest(config, `mutation($input: IssueRelationCreateInput!) {
          issueRelationCreate(input: $input) { success }
        }`, { input: { issueId: issue.id, relatedIssueId: request.other, type: request.type } });
      }
    } else {
      const current = await gqlRequest(config, `query($id: ID!) {
      ...
      for (const request of requests) {
        ...
        const relation = current.issue.relations.find(
          (candidate: any) =>
            candidate.type === request.type && candidate.relatedIssue.identifier === other.identifier,
        );
        if (!relation) {
          throw new ApiError(
