...
describe("planning settings", () => {
  ...
  it("mantiene la validación de resources entre create y update", async () => {
    ...
    const omittedResources = await gql(
      app,
    ...
    const afterInvalid = await gql(app, `query($id: ID!) { initiative(id: $id) { resources } }`, {
      id: initiative.id,
    });
