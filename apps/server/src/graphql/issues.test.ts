...
describe("issueUpdate", () => {
  ...
  it("rechaza estados de otro team y prioridades inválidas", async () => {
    const other = await gql(app, `mutation { teamCreate(input: { name: "Other", key: "OT" }) { team { states { id } } } }`);
    ...
    const badPriority = await gql(app, `
...
describe("issues query + archive", () => {
  ...
  it("resuelve issue por UUID y por identificador", async () => {
    ...
    const viaUuid = await gql(app, `query($id: ID!) { issue(id: $id) { identifier } }`, { id: uuid });
