...
const EXPECTED_POSTGRES_TABLE_ROW =
  "**PostgreSQL** | Se activa con `PRIME_BOARD_PERSISTENCE=postgres` y requiere `PRIME_BOARD_POSTGRES_URL`; usa migraciones independientes `0001`–`0006`. | Mantiene una única Workspace, tiene dominios migrados de forma parcial y usa un SQLite efímero para dominios todavía no migrados. Documents está migrado en `0005`; las demás superficies de planificación siguen teniendo cobertura parcial.";
...
const EXPECTED_POSTGRES_AUDIT_BULLET =
  "- **PostgreSQL** es opcional. Se activa con `PRIME_BOARD_PERSISTENCE=postgres`, requiere `PRIME_BOARD_POSTGRES_URL`, usa migraciones independientes `0001`–`0006` y conserva una única Workspace. La migración es incremental: los dominios no migrados usan un SQLite efímero; Documents ya está disponible en PostgreSQL desde la migración `0005`; la cobertura de suscriptores llega a `0006`.";
