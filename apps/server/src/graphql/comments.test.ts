...
beforeAll(async () => {
  ...
  const actor = await gql(app, `mutation { actorCreate(input: { name: "worker-bot", type: AGENT }) { actor { id } } }`);
  const key = await gql(app, `
  ...
  await gql(app, `mutation { issueCreate(input: { teamKey: "PB", title: "Ship it" }) { issue { id } } }`);
});
afterAll(() => app.stop());
...
describe("comments", () => {
  ...
  it("rechaza cuerpos vacíos e issues inexistentes", async () => {
    const empty = await gql(app, `mutation { commentCreate(input: { issueId: "PB-1", body: "  " }) { success } }`);
    expect(empty.errors?.[0]?.extensions?.code).toBe("VALIDATION_FAILED");
    const missing = await gql(app, `mutation { commentCreate(input: { issueId: "PB-99", body: "x" }) { success } }`);
