...
beforeAll(async () => {
  app = createTestApp();
  const team = await gql(app, `{ team(key: "PB") { id states { id name type position } } }`);
  stateId = team.data!.team.states[0].id;
  completedStateId = team.data!.team.states.find(
...
describe("estados del workflow", () => {
  ...
  it("rechaza convertir el último estado completed a otro tipo", async () => {
    const result = await gql(
      ...
      `
      mutation($id: ID!) {
        workflowStateUpdate(id: $id, input: { type: STARTED }) {
          success
          workflowState { type }
        }
      }
    ...
    expect(
      state.data!.team.states.find(
        (candidate: { id: string }) => candidate.id === completedStateId,
      )!.type,
    ).toBe("COMPLETED");
  });
});
...
describe("borrar estados (AT-164)", () => {
  it("exige destino si el estado tiene issues y los migra con actividad", async () => {
    ...
    const issue = await gql(
      app,
      `query($id: ID!) { issue(id: $id) { state { name } activity { type payload } } }`,
    ...
    const migration = issue.data!.issue.activity.find(
      (a: any) => a.payload?.reason === "state_deleted",
    );
  ...
  it("rechaza destinos de otro team", async () => {
    const other = await gql(
      app,
      `mutation { teamCreate(input: { name: "Ajeno", key: "AJ" }) { team { states { id } } } }`,
    );
    ...
    expect(
      bad.errors === undefined || bad.errors[0]!.extensions?.code === "VALIDATION_FAILED",
    ).toBe(true);
  });
