...
describe("milestones", () => {
  it("crea milestones ordenados dentro de un proyecto", async () => {
    ...
    const list = await gql(
      app,
      `query($id: ID!) { project(id: $id) { milestones { id name position } } }`,
      { id: projectId },
    );
  ...
  it("calcula progreso según issues completados", async () => {
    const before = await gql(
      app,
      `query($id: ID!) { project(id: $id) { milestones { name progress } } }`,
      { id: projectId },
    );
    ...
    const issues = await gql(
      app,
      `query($m: ID!) { issues(filter: { milestone: { eq: $m } }) { nodes { id } } }`,
      { m: m1 },
    );
    await gql(
      app,
  ...
  it("mover el issue de milestone queda registrado en la actividad", async () => {
    ...
    await gql(
      app,
      `mutation($id: ID!, $m: ID!) { issueUpdate(id: $id, input: { milestoneId: $m }) { success } }`,
      { id, m: m2 },
    );
    const activity = await gql(
      app,
      `query($id: ID!) { issue(id: $id) { activity { type } milestone { name } } }`,
      { id },
    );
  ...
  it("borra un milestone y deja sus issues sin milestone (AT-160)", async () => {
    ...
    const after = await gql(
      app,
      `query($id: ID!) { issue(id: $id) { title milestone { name } } }`,
    ...
    const list = await gql(app, `query($id: ID!) { project(id: $id) { milestones { name } } }`, {
      id: projectId,
    });
    expect(list.data!.project.milestones.map((m: any) => m.name)).not.toContain("Descartable");
  });

  it("registra actividad y updatedAt al borrar un milestone dependiente", async () => {
    const createdMilestone = await gql(
      app,
      `
      mutation($p: ID!) { milestoneCreate(input: { projectId: $p, name: "Deleted milestone" }) { milestone { id } } }
    `,
      { p: projectId },
    );
    const milestoneId = createdMilestone.data!.milestoneCreate.milestone.id;
    const issue = await gql(
      app,
      `
      mutation($p: ID!, $m: ID!) {
        issueCreate(input: { teamKey: "PB", title: "Milestone lifecycle", projectId: $p, milestoneId: $m }) {
          issue { id updatedAt }
        }
      }
    `,
    ...
    const after = await gql(
      app,
      `query($id: ID!) {
      issue(id: $id) { updatedAt milestone { name } activity { type actor { name } payload } }
    }`,
      { id: issueId },
    );
    expect(after.data!.issue.milestone).toBeNull();
    ...
    expect(after.data!.issue.activity).toContainEqual({
      ...
      actor: { name: "admin" },
      payload: { from: "Roadmap/Deleted milestone", to: null, reason: "milestone_deleted" },
    });
  });

  it("mantiene el proyecto en un tombstone aunque luego exista un homónimo", async () => {
    const first = await gql(
      app,
      `mutation($p: ID!) { milestoneCreate(input: { projectId: $p, name: "Stable tombstone" }) { milestone { id } } }`,
      { p: projectId },
    );
    const issue = await gql(
      app,
      `mutation($p: ID!, $m: ID!) {
        issueCreate(input: { teamKey: "PB", title: "Stable historical milestone", projectId: $p, milestoneId: $m }) { issue { id } }
      }`,
      { p: projectId, m: first.data!.milestoneCreate.milestone.id },
    );
    await gql(app, `mutation($id: ID!) { milestoneDelete(id: $id) { success } }`, {
      id: first.data!.milestoneCreate.milestone.id,
    });
    await gql(
      app,
      `mutation($p: ID!) { milestoneCreate(input: { projectId: $p, name: "Stable tombstone" }) { success } }`,
      { p: otherProjectId },
    );
    const result = await gql(
      app,
      `query($id: ID!) { issue(id: $id) { activity { type payload } } }`,
      { id: issue.data!.issueCreate.issue.id },
    );
    expect(result.data!.issue.activity).toContainEqual({
      type: "milestone_changed",
      payload: { from: "Roadmap/Stable tombstone", to: null, reason: "milestone_deleted" },
    });
  });
