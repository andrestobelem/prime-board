...
if (values["from-repo"]) {
  const snapshot = join(values["from-repo"], ".prime-board", "meta", "documents.json");
  if (!existsSync(snapshot)) throw new Error(`Documents snapshot not found: ${snapshot}`);
  const result = archiveDocumentSnapshot(snapshot, values.out, "replica");
  console.log(
...
if (config.persistenceBackend === "postgres") {
  ...
  try {
    ...
    const result = archiveDocumentRows(rows, values.out, "postgres");
    console.log(
...
try {
  ...
  >;
  const result = archiveDocumentRows(rows, values.out, "sqlite");
