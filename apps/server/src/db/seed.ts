...
function availableTeamKey(
  ...
  if (!db.query("SELECT id FROM teams WHERE key = ?1").get(preferred)) return preferred;
  if (requested) throw new Error(`Team key already exists: ${preferred}`);
  for (let suffix = 2; ; suffix += 1) {
    const candidate = `${preferred}${suffix}`;
    if (!db.query("SELECT id FROM teams WHERE key = ?1").get(candidate)) return candidate;
  }
}
...
export function seedWorkspace(db: Database, input: WorkspaceSeedInput): WorkspaceSeedResult {
  ...
  db.transaction(() => {
    ...
    const teamId = newId();
    const teamKey = availableTeamKey(db, input.teamKey, firstWorkspace);
    db.query(
    ...
    ).run(newId(), teamId, admin.id, timestamp);
