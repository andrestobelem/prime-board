...
export function listInitiatives(db: Database, includeArchived = false): InitiativeRow[] {
  if (includeArchived) {
    return db.query("SELECT * FROM initiatives ORDER BY created_at").all() as InitiativeRow[];
  }
  return db
    .query("SELECT * FROM initiatives WHERE archived_at IS NULL ORDER BY created_at")
...
function assertCanMutate(existing: InitiativeRow, viewerId: string): void {
...
export function updateInitiative(
  ...
  assertCanMutate(existing, viewerId);
