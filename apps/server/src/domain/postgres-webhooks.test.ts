  ...
  const transaction: PersistenceTransaction = {
    ...
    many: async <Row extends object>(sql: string, params?: SqlParameters) => {
      if (sql.includes("FROM webhooks")) {
        ...
        return rows.filter(
          (row) => row.enabled === true && (ownerId === undefined || row.owner_id === ownerId),
        ) as Row[];
      }
      return [];
    },
...
describe("PostgreSQL webhooks", () => {
  ...
  it("lists only the owner's enabled hooks for a non-admin and deletes by owner", async () => {
    const fake = fakePersistence([
      row,
      { ...row, id: "hook-2", owner_id: "actor-2", enabled: false },
    ]);
    const listed = await listPostgresWebhooks(fake.persistence, {
      id: "actor-1",
      workspace_role: "member",
    });
    expect(listed.map((hook) => hook.id)).toEqual(["hook-1"]);
