...
beforeAll(async () => {
  ...
  const agent = await gql(app, `mutation { actorCreate(input: { name: "worker", type: AGENT }) { actor { id } } }`);
  ...
  `, { p: projectId, m: milestone.data!.milestoneCreate.milestone.id,
       l: [label.data!.labelCreate.label.id], a: agent.data!.actorCreate.actor.id });
  await gql(app, `
    mutation($parent: ID!) {
      issueCreate(input: { teamKey: "PB", title: "Hijo", parentId: $parent }) { success }
    }
  `, { parent: parent.data!.issueCreate.issue.id });
  await gql(app, `mutation { commentCreate(input: { issueId: "PB-1", body: "hola" }) { success } }`);
  const started = team.data!.team.states.find((s: any) => s.type === "STARTED").id;
  await gql(app, `mutation($s: ID!) { issueUpdate(id: "PB-1", input: { stateId: $s }) { success } }`, { s: started });
});
...
describe("rebuildFromRepo", () => {
  ...
  it("preserva relaciones: parent, milestone, labels, comentarios y assignee", () => {
    ...
    expect(parent).toMatchObject({
      assignee: "worker", milestone: "M1", project: "P1", comments: 1, labels: 1,
    });
    fresh.close();
  });
  ...
  it("deriva next_issue_number y preserva las API keys por nombre de actor", () => {
    ...
    fresh.query("INSERT INTO actors (id, name, type, created_at, updated_at) VALUES (?1, 'admin', 'human', 'x', 'x')").run(actorId);
    fresh.query("INSERT INTO api_keys (id, actor_id, name, hash, created_at) VALUES ('k1', ?1, 'mi key', 'HASH123', 'x')").run(actorId);

    const result = rebuildFromRepo(fresh, dir);
    expect(result.preservedKeys).toBe(1);
    const key = fresh.query(
    ...
    const team = fresh.query("SELECT next_issue_number FROM teams WHERE key = 'PB'").get() as { next_issue_number: number };
